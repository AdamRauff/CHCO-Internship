...
[handles] = gui_sinu_plot (time, Pres, iv1PsVal, iv1Time, myfit, ...
...
WaveNumPosRm = find(time(iv1PsIdx)<cp(1));
WaveNumNegRm = find(time(iv1NeIdx)>cp(1));
...
if ~isempty(WaveNumPosRm) && ~isempty(WaveNumNegRm)
    ...
    if ~isempty(WaveRm)
        ...
        [DatStr] = data_isoseg (iv1PsIdx, iv1NeIdx, Oldtime, time, Pres, ...
                                dPmaxIdx, dPminIdx, true, 0, 0);
        ...
        ICS = [Mea Amp Fre Pha];
        [FitStr] = isovol_fit (isoFitPts, time, Pres, ICS);
        ...
        % Plot the results
        [handles] = gui_sinu_plot (time, Pres, iv1PsVal, DatStr.T, ...
...
[handles] = gui_sinu_plot (time, Pres, iv1PsVal, handles.InVar.iv1Time_D, ...
     FitStr.fit, totIsoTimePoints, totIsoPresPoints, hObject, eventdata, ...
     handles); 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%LEGACY%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    [handles] = gui_sinu_plot (time, Pres, iv1PsVal, ...
