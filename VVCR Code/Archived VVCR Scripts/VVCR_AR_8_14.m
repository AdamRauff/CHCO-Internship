...
ORIG_PRES = false; % original pressure vs. time plot
ORIG_DPDT = false; % original dP/dt vs. time plot
...
ISO_PEAKS = false; % plot the isolated peaks using EDP for separation
SINU_FIT = false; % plotting the sinusoidal fitting using ISO_PEAKS points
...
LOC_RNG = false; % display the local ranges found from the time range of pressure waves using EDP
PRES_DPDT_MIN_MAX = true; % plotting dP/dt minima and maxima on pressure data
ES_TO_DPDTMIN = true; % Plot End systolic points and dP/dt Minima on top of pressure curve
...
% tempOverAllRng = 0;
% 
% % determine the longest range of a pressure waveform. this is just used to
% % allocate a matrix
% for i = 1:size(PressPeakRng,2)
%     tempRng = PressPeakRng(2,i) - PressPeakRng(1,i);
%     if tempRng > tempOverAllRng
%         tempOverAllRng = tempRng;
%         PeakInd = i;
%     end
% end
% 
% % pre-allocate
% interval = zeros(length(PressPeakRng(1,PeakInd):0.004:PressPeakRng(2,PeakInd)),size(PressPeakRng,2));
...
%         tempArrLen = length(PressPeakRng(1,i):0.004:PressPeakRng(2,i))
