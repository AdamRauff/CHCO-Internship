...
for i=1:length(Minima)
  if Minima(i)<dPdt_thresh && Minima(i)>-4000 %limiting minima to just the lowest minimums but eliminating the extreme errors
      ...
      timeminall2(Index) = timeplot(MinIdx(i)); %FINDING  the corresponing time for the true minima (dp/dt min)
...
%     for j=1:length(trueMinima) 
%         if deriv(i) == trueMinima(j)
%             timeminall(q)= timeplot(i); %put the corresponding time points in a vector
...
% if DPDT_MIN == true
%     figure; hold on
%     plot(timeplot,deriv,'b',timeminall2,trueMinima,'o');
...
for i=1:length(dpdtminpressureall)
   if dpdtminpressureall(i) > pres_filter_dpdtMin
       dpdtminpressure(ind)=dpdtminpressureall(i);
       timemin(ind)=timeminall2(i);
       ind=ind+1;
   end
end
...
if PRES_FILT == true
    ...
    ylabel('Pressure [mmHg]','FontSize',18);
    xlabel('Time [s]','FontSize',18);
    legend('Original Pressure','Filtered dP/dt Minima', 'All dP/dt Minima');
    box on;
    grid on;
    hold off;
end
...
ind=1;
for i=1:length(Fourier)
    for j=1:length(locstruemax)
        if t(i)==locstruemax(j) %if a time point is equal to the time point at dP/dt max
...
for i=1:length(dpdtmaxpressureall)
    if dpdtmaxpressureall(i) < dPdtMaxPresThresh
        dpdtmaxpressure(ind) = dpdtmaxpressureall(i);
...
if DPDT_MIN_MAX == true
    figure, hold on;
    plot(timeplot,deriv,'b',timeminall2,trueMinima,'ro',locstruemax,pkstruemax,'go');
    % plot pressure peak neighborhoods (vertical lines)
    for i = 1 : size(PressPeakRng,2)
        if PressPeakRng(1,i) ~= 0 && PressPeakRng(2,i) ~= 0
            plot([PressPeakRng(1,i) PressPeakRng(1,i)],[-400 400], 'k');
...
if PRES_DPDT_MIN_MAX == true
    figure, hold on;
    plot(t,Fourier,'b',timemin,dpdtminpressure,'ro',timemax,dpdtmaxpressure,'go');
    set(gca,'fontsize',14);
