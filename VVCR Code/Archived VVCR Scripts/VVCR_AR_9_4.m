...
% --------------------------------------
% Insert UI to remove bad waves *
% Discuss with Melanie and Hunter
% --------------------------------------
...
if length(MinIdx) == length(pksT)
    ...
    % --------------------------------------------
    % shouldn't have a scenario with unequal number of minima and maxima!
    % * see comment about UI prior to EDP calculation
    % --------------------------------------------
end
...
for i=1:n
    ...
    if rem(i,2) == 0
        ...
        for k=1:(n/2) 
            A(k)=(2/n)*pressure(i/2)*cos((i/2*(2*pi)*t(i/2))/n); %A coefficients 
        end
        Asum=2/n+sum(A); %sum of A coeffs

        for k=1:(n/2)
            B(k)=(2/n)*pressure(i/2)*sin((i/2*(2*pi)*t(i/2))/n); % B coefficients
        end
        Bsum=sum(B); %sum of B coeffs

        Fourier(i)=(Asum*cos((i*(2*pi)*t(i/2))/n)) + (Bsum*sin((i*(2*pi)*t(i/2))/n)); 
...
% for ss=1:length(t2)
%    Fourier2(ss)=(Asum*cos((ss*(2*pi)*t2(ss))/n)) + (Bsum*sin((ss*(2*pi)*t2(ss))/n));  
% end

% Now that I have the A and B coefficients saved for each time point, how do I resample???
% t2 = t(1):0.002:t(end);
% n2 = length(t2);
% for i = 1:n2
% 
%   if round(i) ~= 0
%     Fourier2(i) = (Asum(round(i/2))*cos((i*(2*pi)*t2(round(i)))/n)) ...
%      + (Bsum(round(i/2))*sin((i*(2*pi)*t2(round(i)))/n)); 
%   else
%     Fourier2(i) = (Asum(1)*cos((i*(2*pi)*t2(1))/n)) ...
%      + (Bsum(1)*sin((i*(2*pi)*t2(1))/n)); 
%   end
% end
