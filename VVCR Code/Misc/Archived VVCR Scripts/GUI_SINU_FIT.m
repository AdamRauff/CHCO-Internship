function varargout = GUI_SINU_FIT(varargin)
% GUI_SINU_FIT MATLAB code for GUI_SINU_FIT.fig
%      GUI_SINU_FIT, by itself, creates a new GUI_SINU_FIT or raises the existing
%      singleton*.
%
%      H = GUI_SINU_FIT returns the handle to a new GUI_SINU_FIT or the handle to
%      the existing singleton*.
%
%      GUI_SINU_FIT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_SINU_FIT.M with the given input arguments.
%
%      GUI_SINU_FIT('Property','Value',...) creates a new GUI_SINU_FIT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_SINU_FIT_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_SINU_FIT_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_SINU_FIT

% Last Modified by GUIDE v2.5 25-Sep-2016 13:23:37

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_SINU_FIT_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_SINU_FIT_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_SINU_FIT is made visible.
function GUI_SINU_FIT_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_SINU_FIT (see VARARGIN)

% Choose default command line output for GUI_SINU_FIT
...
% UIWAIT makes GUI_SINU_FIT wait for user response (see UIRESUME)
...
if ~isempty(WaveNumPosRm) && ~isempty(WaveNumNegRm)
    ...
    if ~isempty(WaveRm)
        ...
        % erase wave from isovoltime structure
        isovoltime(WaveRm).PosIso = [];
        isovoltime(WaveRm).NegIso = [];
        EDP_T(WaveRm) = [];
        EDP_NT(WaveRm) = [];
        
        % create isovol structure. this structure will hold the pressure
        % values corresponding
        isovol = struct('PosIso',cell(length(EDP_T),1),'NegIso',cell(length(EDP_T),1));
        for i = 1:length(EDP_T)
            for j = 1:length(isovoltime(i).PosIso)
                isovol(i).PosIso(j,1) = Pres(isovoltime(i).PosIso(j,1));
            end
        ...
        % pre - allocate 
        waveFit = zeros(length(EDP_T),1);
        PmaxT = zeros(length(EDP_T),1);
        P_max2 = zeros(length(EDP_T),1);
        r_square2 = zeros(length(EDP_T),1);
        c_tot2 = zeros(length(EDP_T),4);
...
function varargout = GUI_SINU_FIT_OutputFcn(~, ~, handles) 
