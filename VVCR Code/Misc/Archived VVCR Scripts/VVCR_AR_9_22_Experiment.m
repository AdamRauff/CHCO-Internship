...
SINU_FIT = true; % display sinusoidal fit in its own figure after Sinu fit GUI
...
for i = 1:length(pksT)
    ...
    % find EDP as where dP/dt exceeds 20% of its maximum
    EDi = pksT(i);
...
%% This is the experiment
    % expand isovolmic range to: from 0.2*max prior to max, to 0.3*max after max
    IVTi = pksT(i);
    while dPdt(IVTi) > 0.30*pks(i)
        IVTi = IVTi + 1;
    end
    ...
    % find point on the other side of the pressure wave, with the
    % same height (pressure) as IVT. IVT_N - IVT Negative
    IVT_Ni = IVTi+5;
    
    % round values to nearest tenth
    while round(Pres(IVT_Ni),1) > round(IVT_T(i),1)
        IVT_Ni = IVT_Ni+1;
    end

    % find which point is closest to the pressure value of IVT
    tempNeg_IVTs = IVT_Ni-3:1:IVT_Ni+3; % create local neighborhood

    % calculate the difference between the local neighborhood
    IVT_Diffs = abs(IVT(i)-Pres(tempNeg_IVTs));

    % find minimum difference
    [~, tempInds] = min(IVT_Diffs);

    % assign Negative EDP values
    IVT_NT(i) = tempNeg_IVTs(tempInds); % keep in mind this is an indicie of the time vector
    IVT_N(i) = Pres(tempNeg_IVTs(tempInds)); % pressure in mmHg
    % -----------------------------------------------------------------------------
end
...
clear EDP_Diffs tempNeg_EDPs tempNum tempNeg_IVTs
